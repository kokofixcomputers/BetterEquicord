...
import DiscordModules from "./local/discordmodules.js";
import { arrayToObject, compat_logger, createTextForm } from "./utils.js";

// Expose modules to plugins
window.DiscordModules = DiscordModules;
window.ContextMenu = {
...
function wrapBetterDiscordPluginCode(pluginCode: string, filename: string) {
    ...
    const debugLine =
        "\ntry{" +
        codeData +
        `}catch(e){console.error("BDCompat wrapper error in ${filename}:", e);debugger;}`;
    const additionalCode = [
        "const module = { exports: {} };",
        "const exports = module.exports;",
        "const global = window;",
        "const __bdPluginsFolder = (BdApi.Plugins.folder ?? \"/BetterDiscordPlugins\");",
        "const __filename = __bdPluginsFolder + `/" + filename + "`;",
        "const __dirname = __bdPluginsFolder;",
        "const DiscordNative = { get clipboard() { return window.BdCompatLayer.fakeClipboard; } };",
        "const DiscordModules = window.DiscordModules;",
    ];
    codeData =
        "(()=>{" +
        additionalCode.join("") +
